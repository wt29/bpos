...
ReTrans - A process to run in the regional server to do file polling and updates
...
local mfiles, i, aArray, mpoll_int, mpoll_comp, mRAS
...
mRAS := if( type( "noras" ) = 'U', TRUE, FALSE )
...
while TRUE
 ...
 while !branches->( eof() ) 
  ...
  //************* Transaction Types
    mtranarr := { 'stk', 'sls', 'bnk', 'rcv', 'trf', 'por', 'ttl' }
  ...
  // Put data back to the branches here
    stkfiles := directory( 'to' + trim( branches->branch ) + '\stk?????.dbf' )
    if !empty( stkfiles ) 
     if Netuse( 'i:stock' )
      ordsetfocus( BY_ID )
       for y := 1 to len( stkfiles )
        if Netuse( 'to' + trim( branches->branch ) + '\' + stkfiles[ y, 1 ], EXCLUSIVE, , 'stkupdt' )
	 while !stkupdt->( eof() )
	  if !stock->( dbseek( stkupdt->id + stkupdt->branch ) )
	   Add_rec('stock')
	   stock->id := stkupdt->id 
	   stock->branch := stkupdt->branch
	  endif
	 ...
	 stkupdt->( dbclosearea() )
        ...
        kill( 'to' + trim( branches->branch ) + '\' + stkfiles[ y, 1 ] )
       next
      stock->( dbclosearea() )
     endif
    endif
...
*/
 ...
 while !branches->( eof() )
  for x := 1 to len( downfiles )
   copy file ( 'fromho\' + downfiles[ x, 1 ] ) to ( 'to' + branches->branch + '\' + downfiles[ x, 1 ] )
  next x
  branches->( dbskip() )
 enddo  
...
*
func params_maint
local mscr := Box_Save( 2, 10, 5, 50 ), getlist := {}
if Netuse( 'reparams')
...
oTBrowse:addcolumn( tbcolumnnew( "Store Code", { || branches->branch } ) )
go top
keybuff:=''
while TRUE
 ...
 if !navigate( oTBrowse, key )
  ...
  case key == K_F10 .or. key == K_INS
   mscr := Box_Save( 09, 01, 14, 72, C_GREY )
   if key == K_INS
    branches->( dbappend() )
   endif 
   ...
   @ 13, 03 say ' Store Code' get branches->branch pict '!!!' ;
	    valid !empty( branches->branch )
   read
   if empty( directory( 'to' + trim( branches->branch) , 'D' ) )
    Shell( 'md ' + 'to' + trim( branches->branch ) )
   endif
   Box_Restore( mscr )
   oTBrowse:refreshall()
  endcase
...
return nil
*
function Box_Save ( t, l, b, r )   // top,left,bottom,right,colour  // the Array version
...
*
Function Box_Restore ( marray )    // the array version of above
...
mscr := Box_Save( p_row, p_col, p_row+3, p_col+( plen*2 )+3 , 3, pcolor )
...
next
mscr := Box_Save( 24-3-len( aArray ) , 79-4-mwidth, 24-2, 79-2, C_YELLOW )
for x := 1 to len( aArray )
...
if sbox1 = nil
 sbox1 := Box_Save( 2, 1, 17, 78 )
 sbox2 := Box_Save( 18, 2, 24, 78 )
